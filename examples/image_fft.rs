...
fn main() {
    ...
    let img = ImageReader::open("img/cameraman.pgm").unwrap().decode().unwrap();
    let gray_img = img.to_luma8();
    gray_img.save(format!("img/output/gray_img.jpg")).unwrap();

    let fft_img = &mut fft_2d(gray_img);
    fft_img.save(format!("img/output/fft_img.jpg")).unwrap();

    let fft_img_shift = &mut fft_shift(fft_img);
    fft_img_shift.save(format!("img/output/fft_img_shift.jpg")).unwrap();

    let fft_reverse_img_shift = fft_shift(fft_img_shift);
    fft_reverse_img_shift  .save(format!("img/output/fft_reverse_img_shift.jpg")).unwrap();

    let ifft_img = &mut ifft_2d(fft_reverse_img_shift);
    ifft_img.save(format!("img/output/ifft_img.jpg")).unwrap();

    let duration = start.elapsed();
    println!("Time elapsed to do 2-D fft and shift of image is: {:?}", duration);
}

fn fft_shift(img: &mut GrayImage) -> GrayImage {
